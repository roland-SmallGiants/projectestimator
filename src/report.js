...
export function renderNeverUsedSection() {
  ...
  el.innerHTML = groups.map((g) => {
    ...
    const headerRow = `<tr class="never-used-discipline-header" data-category="${esc(g.category)}" style="cursor:pointer;">
      <td style="font-weight:700; color:var(--accent); padding-top:14px;"><span style="display:inline-block; width:14px;">${isOpen ? "\u25be" : "\u25b8"}</span>${esc(g.category)}${g.allUnused ? ` <span class="sub" style="color:var(--rose); font-weight:600;">\u2014 no tasks in this deliverable have been quoted yet</span>` : ""}</td>
    </tr>`;
